...
@app.route("/conformation")
def order():
  return render_template('conformation.html',name='Home')
...
def addproduct():
  if request.method == 'POST':
    ...
    if types =='tracks':
      insert_sql = "INSERT INTO TRACK(name, image, categorie, rate) VALUES (?,?,?,?)"
      prep_stmt = ibm_db.prepare(conn, insert_sql)
      ibm_db.bind_param(prep_stmt, 1, name)
      ibm_db.bind_param(prep_stmt, 2, image)
      ibm_db.bind_param(prep_stmt, 3, categorie)
...
#selecting_hat
  ...
  sql3="SELECT * FROM TRACK"
  stmt3 = ibm_db.exec_immediate(conn, sql3)
  tracks=ibm_db.fetch_both(stmt3)
  while tracks != False :
  ...
  #returning to HTML
  return render_template('home.html',dictionary= shirt_list,pants=pant_list,hats=hat_list,sarees=saree_list,tshirts=tshirt_list,topss=tops_list,leggings=leggings_list,tacks=track_list)
...
#selecting_watch
  ...
  print(track_list) 
  #returning to HTML
  return render_template('home.html',dictionary= shirt_list,pants=pant_list,hats=hat_list,sarees=saree_list,tshirts=tshirt_list,topss=tops_list,leggings=leggings_list,tracks=track_list)
...
def dis():
  if request.method == 'POST':
    ...
    rate=request.form['rate']
    categorie=request.form['categorie']
  return render_template('conformation.html',pname=pname,img=img,rate=rate,categorie=categorie)
