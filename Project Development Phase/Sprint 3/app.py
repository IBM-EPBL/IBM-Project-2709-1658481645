...
def dis():
  if request.method == 'POST':
    ...
    categorie=request.form['categorie']
    rate=request.form['rate']
  return render_template('conformation.html',pname=pname,img=img,categorie=categorie,rate=rate)
...
def orderdisplay():
  if request.method == 'POST':
    ...
    insert_sql = "INSERT INTO DISPLAYORDER(name, image, categorie, rate) VALUES (?,?,?,?)"
    prep_stmt = ibm_db.prepare(conn, insert_sql)
    ibm_db.bind_param(prep_stmt, 1, name)
    ibm_db.bind_param(prep_stmt, 2, image)
    ibm_db.bind_param(prep_stmt, 3, categorie)
...
def displayorder():
  ...
  while detail != False :
      ...
      detail = ibm_db.fetch_both(stmt)
  print(details_list)
  return render_template('disorder.html',details=details_list)
